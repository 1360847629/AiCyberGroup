...
import { getEntities } from './job-report.reducer';
...
export const JobReport = () => {
  ...
  const jobReportList = useAppSelector(state => state.jobReport.entities);
  const loading = useAppSelector(state => state.jobReport.loading);
  const totalItems = useAppSelector(state => state.jobReport.totalItems);
  ...
  return (
    <div>
      <h2 id="job-report-heading" data-cy="JobReportHeading">
        <Translate contentKey="cyberClinicApp.jobReport.home.title">Job Reports</Translate>
      ...
      <div className="table-responsive">
        {jobReportList && jobReportList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="cyberClinicApp.jobReport.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th>
                  <Translate contentKey="cyberClinicApp.jobReport.jobRequest">Job Request</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
            ...
            <tbody>
              {jobReportList.map((jobReport, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  <td>
                    <Button tag={Link} to={`/job-report/${jobReport.id}`} color="link" size="sm">
                      {jobReport.id}
                    </Button>
                  </td>
                  <td>{jobReport.reportDate ? <TextFormat type="date" value={jobReport.reportDate} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{jobReport.details}</td>
                  <td>{jobReport.isSuccessful ? 'true' : 'false'}</td>
                  <td>
                  ...
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`/job-report/${jobReport.id}`} color="info" size="sm" data-cy="entityDetailsButton">
                      ...
                      <Button
                        tag={Link}
                        to={`/job-report/${jobReport.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="cyberClinicApp.jobReport.home.notFound">No Job Reports found</Translate>
            </div>
          )
        )}
      </div>
      {totalItems ? (
        <div className={jobReportList && jobReportList.length > 0 ? '' : 'd-none'}>
